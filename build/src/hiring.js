...
async function waitForJobVerification(jobId, provider) {
  ...
  const abi = sdk_core_1.AbiRegistry.create(
    JSON.parse(fs.readFileSync(abiPath, 'utf8')),
  );
  for (let i = 0; i < maxRetries; i++) {
    process.stdout.write('.');
    try {
      const serializer = new sdk_core_1.ArgSerializer();
...
async function submitReputation(jobId, rating, provider, signer, sender) {
  ...
  const factory = new sdk_core_1.SmartContractTransactionsFactory({
    abi,
    config: new sdk_core_1.TransactionsFactoryConfig({
      chainID: config_1.CONFIG.CHAIN_ID,
    }),
  });
  const endpoint = abi.getEndpoint('submit_feedback');
  const typedArgs = sdk_core_1.NativeSerializer.nativeToTypedValues(
    [
      Buffer.from(jobId),
      BigInt(agentNonce),
      new sdk_core_1.BigUIntValue(BigInt(rating)),
    ],
    endpoint,
  );
