...
async function main() {
  ...
  const data = Buffer.from(
    `register_agent@${nameHex}@${uriHex}@${publicKeyHex}${metadataHex}`,
  );
  ...
  if (useRelayer) {
    ...
    try {
      // A. Get Challenge
      const {data: challenge} = await axios_1.default.post(
        `${config_1.CONFIG.PROVIDERS.RELAYER_URL}/challenge`,
        {
      ...
      let relayerAddressBech32 = RelayerAddressCache_1.RelayerAddressCache.get(
      ...
      if (!relayerAddressBech32) {
        ...
        try {
          const {data} = await axios_1.default.get(
            `${config_1.CONFIG.PROVIDERS.RELAYER_URL}/relayer/address/${senderAddress.toBech32()}`,
          );
          relayerAddressBech32 = data.relayerAddress;
          RelayerAddressCache_1.RelayerAddressCache.set(
            config_1.CONFIG.PROVIDERS.RELAYER_URL,
            senderAddress.toBech32(),
          ...
          console.warn(
            `Failed to fetch specific relayer address: ${e.message}. Proceeding without explicit relayer field (may fail if V3 strict).`,
          );
        }
      } else {
      ...
      const {data: relayResult} = await axios_1.default.post(
        `${config_1.CONFIG.PROVIDERS.RELAYER_URL}/relay`,
        {
      ...
      console.log(
        `Check Explorer: ${config_1.CONFIG.EXPLORER_URL}/transactions/${relayResult.txHash}`,
      );
    } catch (e) {
      const err = e;
      console.error(
    ...
    try {
      ...
      console.log(
        `Check Explorer: ${config_1.CONFIG.EXPLORER_URL}/transactions/${txHash}`,
      );
    } catch (e) {
