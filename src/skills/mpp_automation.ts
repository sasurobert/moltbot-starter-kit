export async function fundSessionFromDiscovery(
  identitySkill: {getAgentPricing: (address: string) => Promise<bigint>},
  mppSkill: {
    openSession: (
      address: string,
      price: bigint,
  ...
  const txHash = await mppSkill.openSession(
    agentAddress,
    priceInt,
    tokenIdentifier,
    durationSeconds,
...
export async function slashSessionOnFeedback(
  ...
  mppSkill: {requestCloseSession: (channelId: string) => Promise<string>},
  agentAddress: string,
  rating: number,
  jobId: string,
  channelId: string,
  ...
  const feedbackTx = await identitySkill.submitFeedback(
    agentAddress,
    rating,
    jobId,
  );
